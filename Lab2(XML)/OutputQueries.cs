...
namespace LabXML
{
    ...
    public static class OutputQueries
    {
        ...
        public static void ShowMarks(IEnumerable<Exam> marks, int min, int max)
        {
        ...
        public static void ShowStudentsMarks(IEnumerable<(string, decimal)> tuples)
        {
            WriteSubtitle("Перелік всіх студентів та їх оцінки");
            foreach ((string student, decimal mark) in tuples)
            {
                Console.WriteLine($"{student}: {mark}");
            }
